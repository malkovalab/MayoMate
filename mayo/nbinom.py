...
def get_genome_size() -> int:
    ...
    from mayo.settings import chromosomes
    genome_size = 0
...
def findClusterProbability(
    ...
    #print(f"p={cdf:.8e} for cluster length {cluster_length:8} with {number_of_mutations:3} mutations; with per bp mut probability {per_bp_mut_probability}")
    return cdf
...
def analyzeSampleForMutationClusters(
    ...
    per_site_probability = total_outcome_mutations/get_genome_size()
...
if __name__ == "__main__":
    ...
    from mayo import load_payload_from_pickle
    import logging
    ...
    df_mutations = load_payload_from_pickle(f"outputs/partial_pickles/output_df.pkl")
    df_clusters = analyzeSampleForMutationClusters(df_mutations, min_p_value=0.005)
